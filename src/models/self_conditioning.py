...
from src.utils.embedding import rbf_twoscale, _rbf
from src.models.gvp import _norm_no_nan
...
class SelfConditioningResidualLayer(nn.Module):
    ...
    def forward(self, 
        ...
        d_to_target = _norm_no_nan(x_t - dst_dict['x']) # has shape n_nodes x 1 (hopefully)
        d_to_target = _rbf(d_to_target, D_max=self.rbf_dmax, D_count=self.rbf_dim)

        d_to_source = _norm_no_nan(x_t - x_0) # has shape n_nodes x 1 (hopefully)
        d_to_source = _rbf(d_to_source, D_max=self.rbf_dmax, D_count=self.rbf_dim)
        ...
        if 'c' in dst_dict:
            node_residual_inputs.append(dst_dict['c'])
